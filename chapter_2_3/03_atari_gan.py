...
def iterate_batches(envs, batch_size=BATCH_SIZE):
    batch = []
    env_states = [False] * len(envs)  # Track if env needs reset
    expected_shape = envs[0].observation_space.shape

    while True:
        idx = random.randrange(len(envs))
        e = envs[idx]
        if not env_states[idx]:
            obs = e.reset()
            env_states[idx] = True
        obs, reward, terminated, truncated, _ = e.step(e.action_space.sample())
        is_done = terminated or truncated
        if np.mean(obs) > 0.01 and obs.shape == expected_shape:
            batch.append(obs)
